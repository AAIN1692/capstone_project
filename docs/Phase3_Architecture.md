# Technical Architecture — Sales Pulse
...
**Note on SQLite in production:** Railway's filesystem is ephemeral on redeploys. Mitigation:
either (a) use a Railway persistent volume for the SQLite file, or (b) re-run the seed script
on container start so the app is always self-consistent even if the file resets. Documented
as an accepted tradeoff for MVP — flagged again in Section 6 (Risks).
...
---
*Next: Phase 4 — Vibe Coding Spec (8 sections) + Annotated Prompt Library, built from this architecture.*
